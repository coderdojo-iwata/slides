...
const truncateAttribute = (target, maxLength = 30) =>
...
const descSortedLinks = fs
  ...
  .map(
    (pathWithoutExtension) =>
      `            <a href="${pathWithoutExtension}.html" class="bg-cover bg-center h-64 rounded-lg shadow-md hover:shadow-lg transform transition duration-300 hover:scale-105"
                style="background-image: url('${pathWithoutExtension}.png'); background-size: contain; background-repeat: no-repeat; background-position: center;">
                <div class="bg-gray-700 bg-opacity-50 h-full w-full rounded-lg p-4 flex items-end">
                    <h2 class="text-white text-sm font-semibold">${truncateAttribute(
